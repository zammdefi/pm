...
contract PMHookRouterEdgeCasesTest is Test {
    ...
    PMFeeHookV1Minimal public hook;
    ...
    function test_LifetimeTWAP_ManipulationResistance() public {
        ...
        vm.prank(BOB);
        router.buyWithBootstrap{value: 1 ether}(
            marketId, false, 1 ether, 0, BOB, block.timestamp + 1 hours
        );
        ...
        vm.prank(ALICE);
        router.buyWithBootstrap{value: 1 ether}(
            marketId, false, 1 ether, 0, ALICE, block.timestamp + 1 hours
        );
...
contract PMHookRouterAdvancedEdgeCasesTest is Test {
    PMHookRouter public router;
    PMFeeHookV1Configurable public hook;
    ...
    /// @notice Test hook fee at 10000 bps (100% - invalid, should use fallback)
    ...
    /// @notice Test hook fee > 10000 bps (invalid, should use fallback)
